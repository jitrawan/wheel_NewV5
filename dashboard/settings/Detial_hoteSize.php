...
<div style="margin: 10px;">
	   <div id="searchOther" name="searchOther" style="display: none">
			  <div class="form-group row">
						<div class="col-md-6">
							<input type="hidden" name="id" id="id" class="form-control" value="<?php echo @$_GET['key'];?>">
							<label for="detail_holeSize">รู</label>
								<select name="detail_holeSize" id="detail_holeSize" class="form-control">
									...
									<? $getHoleSizeWhee = $getdata->my_sql_select(NULL,"HoleSizeWhee","status = '1' ORDER BY id ");
										...
										<option value="<?= $showHoleSizeWhee->id?>" ><?= $showHoleSizeWhee->Description?></option>
										<?}?>
								</select>
							</div>
						</div>
						<button type="submit" name="save_hoteSize" class="btn btn-primary btn-sm"><i class="fa fa-save fa-fw"></i><?php echo @LA_BTN_SAVE;?></button>
			</div>
	    </div>
   ...
   <?php
   $x=0;
$getcat = $getdata->my_sql_select(" s.id as id, s.HoleSizeWhee as HoleSizeWhee"," DiameterWhee w
left join relationHoleSize s on w.id = s.HoleSizeId "
," w.id = '".addslashes($_GET['key'])."' ");
   ...
   <tr id="<?php echo @$showcat->id;?>">
     ...
     <td>&nbsp;<?php $getWidth =$getdata->my_sql_query(NULL,"HoleSizeWhee","id='".@$showcat->HoleSizeWhee."'"); echo $getWidth->code ?></td>
     <td>&nbsp;<?php $getWidth =$getdata->my_sql_query(NULL,"HoleSizeWhee","id='".@$showcat->HoleSizeWhee."'"); echo $getWidth->Description ?></td>
...
function deletecat(catkey){
	...
	xmlhttp.open("GET","function.php?type=delete_detailHoteSizewheel&key="+catkey,true);
	xmlhttp.send();
	}
}
