...
<div style="margin: 10px;">
			  ...
			  <div class="form-group row">
						<div class="col-md-6">
							<input type="hidden" name="id" id="id" class="form-control" value="<?php echo @$_GET['key'];?>">
								<label for="detail_SeriesRubble">ซี่รี่</label>
								<select name="detail_SeriesRubble" id="detail_SeriesRubble" class="form-control">
						...
						<button type="submit" name="save_detailseries" class="btn btn-primary btn-sm"><i class="fa fa-save fa-fw"></i><?php echo @LA_BTN_SAVE;?></button>
			</div>
	    </div>
   ...
   <?php
   $x=0;
$getcat = $getdata->my_sql_select(" s.id as id, s.SeriesRubble as SeriesRubble"," WidthRubble w
left join relationSeries s on w.id = s.SeriesId "
," w.id = '".addslashes($_GET['key'])."' ");
   ...
   <tr id="<?php echo @$showcat->id;?>">
     ...
     <td>&nbsp;<?php $getWidth =$getdata->my_sql_query(NULL,"SeriesRubble","id='".@$showcat->SeriesRubble."'"); echo $getWidth->code ?></td>
...
function deletecat(catkey){
	...
	xmlhttp.open("GET","function.php?type=delete_detailSeriesRubble&key="+catkey,true);
	xmlhttp.send();
	}
}
