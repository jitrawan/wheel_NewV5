...
<div style="margin: 10px;">
	   <div id="searchOther" name="searchOther" style="display: none">
			  <div class="form-group row">
					<div class="col-md-6">
							...
							<label for="detail_DiameterRubble">ขนาด</label>
							<select name="detail_DiameterRubble" id="detail_DiameterRubble" class="form-control">
						...
						<button type="submit" name="save_detaildiameter" class="btn btn-primary btn-sm"><i class="fa fa-save fa-fw"></i><?php echo @LA_BTN_SAVE;?></button>
			</div>
	    </div>
   ...
   <?php
   $x=0;
$getcat = $getdata->my_sql_select(" s.id as id, s.DiameterRubble as DiameterRubble"," WidthRubble w
left join relationDiameter s on w.id = s.DiameterId "
," w.id = '".addslashes($_GET['key'])."' ");
   ...
   <tr id="<?php echo @$showcat->id;?>">
     ...
     <td>&nbsp;<?php $getWidth =$getdata->my_sql_query(NULL,"DiameterRubble","id='".@$showcat->DiameterRubble."'"); echo $getWidth->code ?></td>
   ...
   ?>
		...
		<div class="modal-footer">
				<button type="button" name="save_detaildiameter" class="btn btn-default btn-sm" data-dismiss="modal"><i class="fa fa-times fa-fw"></i><?php echo @LA_BTN_CLOSE;?></button>
...
function deletecat(catkey){
	...
	xmlhttp.open("GET","function.php?type=delete_detailDiameterRubble&key="+catkey,true);
	xmlhttp.send();
	}
}
